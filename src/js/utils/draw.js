...
function makeVertLine(x, label, y1, y2, options={}) {
	...
	var texts=[];
	if(options.splitInLines){		
		var labelWords = label.split(" ");

		if(labelWords.length > 1){	
			var firstWord = labelWords[0];
			var secondWord = labelWords[1];

			for(var idx=2; idx < labelWords.length; idx++)
			{
				secondWord+= " " +labelWords[idx];
			}

			texts.push(createSVG('text', {
				x: 0,
	...
	line.appendChild(l);
	texts.forEach(elem => line.appendChild(elem));
...
export function xLine(x, label, height, options={}) {
	...
	if(!options.stroke) options.stroke = BASE_LINE_COLOR;
	if(!options.className) options.className = '';
	if(!options.splitInLines) options.splitInLines = 0;
	...
	return makeVertLine(x, label, y1, y2, {
		...
		className: options.className,
		lineType: options.lineType,
		splitInLines: options.splitInLines
	});
}
